	...
	function autofillTabeleMagacin_saradnici(id){
		...
		xhr.open("get", "ajax_response.php?id_magacina="+id_magacina+"&page=magacin", false);
		xhr.send();
	...
	function createNewInput(i,id_magacina){
		...
		var kolicinaPredhodnog = $('#kolicina'+i).val();
		var sledeciArtikal = $('#proizvod'+s).val(); //vrednost u sledecem inputu
		
		if (predhodniArtikal != "" && kolicinaPredhodnog != ""){ // ako predhodni input nije prazan izpisuje se novi input
			i = s;
			
			var div_start = '<div class="porudzbenica-artikli" id="'+i+'">';
			var cell1 = '<div class="redni-broj">'+i+'. </div>';
			var cell2 = '<input type="text" class="awesomplete center-text proizvod-input" name="proizvod'+i+'" id="proizvod'+i+'" list="proizvodi" size="28" placeholder="Izaberi Artikal" onChange="autofillProizvoda(this,'+"'"+i+"','narudzbenica','"+id_magacina+"')"+'">';
			var cell3 = '<input type="text" class="center-text input-small" onblur="calculateSum()" name="kolicina'+i+'" id="kolicina'+i+'" size="5" placeholder="kolicina" >';			
			var cell4 = '<input type="text" class="center-text input-small" onblur="calculateSum()" name="cena_proizvoda'+i+'" id="cena-proizvoda'+i+'" size="5" placeholder="cena" >';

			var cell5 = '<input type="text" class="center-text input-small" onblur="calculateSum()" name="tezina'+i+'" id="tezina'+i+'" size="5" placeholder="tezina" >';
			var cell6 = '<input type="text" class="center-text input-small" onblur="calculateSum()" name="broj-paketa'+i+'" id="broj-paketa'+i+'" size="3" placeholder="paketi" >';
			...
			var cell9 = '<div class="broj center-text input-small plus" id="minus'+i+'" onclick="deleteInput('+"'"+i+"','"+id_magacina+"')"+'">-</div>';
			var div_end = '</div>';		

			$('#proizvodi-za-porudzbinu').append(div_start+cell1+cell2+cell3+cell4+cell5+cell6+cell7+cell8+cell9+div_end);
			$("#broj_artikala").val(i);
			...
			if (predhodniArtikal == "") { alert("Morate odabrati artikal da biste presli na sledeci");}
			if (kolicinaPredhodnog =="") { alert("Unesite kolicinu pre nego sto predjete na sledeci artikal");}
		}

		function load_js(){ //Naknadno ucitavanje JS fajla awesomplete.js
	...
	function deleteInput(i,id_magacina){
		/*brisanje unosa artikla*/
		$("#"+i).remove();

		/*prepravka broja unetih artikala (u sakrivenom input polju)*/
		var old_broj_artikala = $("#broj_artikala").val();
		var broj_artikala = old_broj_artikala - 1;
		$("#broj_artikala").val(broj_artikala);

		
		var predhodni = i-1; /*id kolone prethodnog artikla*/
		var plus = '<div class="broj center-text input-small plus" id="plus'+predhodni+'" onclick="createNewInput('+"'"+predhodni+"','"+id_magacina+"')"+'">+</div>';
		var minus = '<div class="broj center-text input-small plus" id="minus'+predhodni+'" onclick="deleteInput('+"'"+predhodni+"','"+id_magacina+"')"+'">-</div>';

		/*ako artikal (koji je pre obrisanog artika) nije prvi dodaje mu se "+" i "-" */
		if(broj_artikala > 1){
			$("#"+predhodni).append(plus + minus);
		}
		else{
			$("#"+predhodni).append(plus);
		}
